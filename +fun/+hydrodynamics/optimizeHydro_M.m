...
%               nvnguyen@umass.edu
%-------------------------------------------------------------------------%
function [miu, nu, nu_pto, F, P, Cf_opt, Cg] = optimizeHydro_M(g, rho, T, Aw, Asc, h,...
                                                    w, c, I, C, n, nmax) 
...
for i = 1:ncases
    ...
    for j = 1:n
        ...
        miu_temp = miu_temp + fn(i,j)*fn(i,j)*imag(vtemp);
        if (j == 1)
            nu_temp = real(vtemp);
            f_temp = vtemp;
        end
    end
    miu(i) = rho*w*w*pi*miu_temp;
    nu(i) = -rho*omega(i)*w*w*f0(i)*f0(i)*pi*nu_temp;
    F(i) = rho*omega(i)*Aw*w*w*f0(i)*d0(i)*pi*f_temp;
end
